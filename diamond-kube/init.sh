...
# and copies them to diamond-kube/ folder
if [ "$#" -lt 2 ]; then
...
# copy stuff diamond-kube folder (as helm charts cannot access files outside of chart folder)
# see https://github.com/helm/helm/issues/3276#issuecomment-479117753
cd $current_folder

rm -rf diamond-kube/crypto-config
rm -rf diamond-kube/channel-artifacts

cp -r $project_folder/crypto-config diamond-kube/
cp -r $project_folder/channel-artifacts diamond-kube/

cp -r $project_folder/configtx.yaml diamond-kube/
